...
\begin{wrapfigure}{r}{0.4\textwidth}
    \includegraphics[height=12em]{gh-edge-problem}
...
perhapes some kind of 2D mapping from pairs of node IDs (or a base node and
edge node) to the cell array. However, it is important to note that we don't
actually need to know if GraphHopper internally stores an edge as forward or
reverse so long as we are able to distinguish between the two cases shown above.
...
(\texttt{boolean[][] cells} and \texttt{boolean[][] reverseCells})
 and uses whichever one is appropriate for the current EdgeIteratorState.
...
%When routing, GraphHopper starts by finding the nearest edge to the start
%and end locations. It then creates `virtual edges' from the start location to
%the first edge and from the last edge to the
...
\begin{wrapfigure}{r}{0.4\textwidth}
    \vspace{-2em}
    \centering
    \includegraphics[width=0.35\textwidth]{dijkstra-15k-wide}
